...
// 输入后1秒或失去焦点时保存
editor.on('update', useDebounceFn(({ editor }: EditorEvents['update']) => handleDebouncedSave(eventId, editor.getHTML())))
editor.on('blur', ({ editor }: EditorEvents['blur']) => handleSave(eventId, editor.getHTML()))

const { data: content, isSuccess, isLoading, isError } = $(useQuery({
  enabled: computed(() => eventId !== undefined),
  queryKey: computed(() => ['event', eventId, 'content']),
  queryFn: () => api.event.getContent(eventId!),
...
watch(
  ...
  (newContent, oldContent) => {
    ...
    if (oldContent?.eventId === newContent?.eventId || !newContent) { return }
    editor.commands.setContent(newContent.content)
  },
)

const handleSet = () => {
  editor.commands.setContent('<p><span data-type=\"mention\" data-id=\"@1\">114514</span></p>')
}
</script>
